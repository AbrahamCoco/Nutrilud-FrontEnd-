...
const comidas = ["Desayuno", "Almuerzo", "Media tarde", "Cena", "Colación"];

export default function Recordatorios() {
  const searchParams = useSearchParams();

  // Estado para cada campo editable
  const [hora, setHora] = useState(Array(comidas.length).fill(""));
  const [lugar, setLugar] = useState(Array(comidas.length).fill(""));
  const [alimentos, setAlimentos] = useState(Array(comidas.length).fill(""));
  const [porciones, setPorciones] = useState(Array(comidas.length).fill(""));
  const [marca, setMarca] = useState(Array(comidas.length).fill(""));
  const [preparacion, setPreparacion] = useState(Array(comidas.length).fill(""));
  ...
  const handleSubmit = () => {
    const json = {
      nutriologo_id: 1, // Puedes reemplazarlo con un ID real si lo tienes en contexto
      paciente_id: 1,
      otros,
      observaciones,
      tdata_recordatorio: {
    ...
    <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 mt-6">
          <button onClick={() => window.history.back()} className="flex items-center gap-2 px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition-colors">
            <FaArrowLeft />
            Regresar
          </button>
          <button
