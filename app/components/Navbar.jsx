...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const nombreUsuario = `${sessionStorage.getItem('nombre') || ''} ${
      sessionStorage.getItem('primer_apellido') || ''
    }`;
            ...
            <div className="flex items-center">
              {rol ? (
                <div className="flex items-center space-x-4">
                  <Link
                    href={`/perfil/${id}/${rol}`}
                    className="hidden md:block text-green-100 hover:text-white px-3 py-2 text-sm font-medium transition-colors duration-200"
                  >
                    Mi Perfil
                  </Link>
