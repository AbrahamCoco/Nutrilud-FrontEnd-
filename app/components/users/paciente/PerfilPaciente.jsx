...
export default async function PerfilPaciente({ id }) {
  ...
  const pacienteData = userData?.tusuario_pacientes;
  const edad = await PerfilController.calcularEdad(pacienteData?.fecha_nacimiento);
  const ultimaConsulta = pacienteData?.consulta?.length > 0 ? pacienteData.consulta[pacienteData.consulta.length - 1] : null;

  const fechaNacimiento = pacienteData?.fecha_nacimiento
    ? new Date(pacienteData.fecha_nacimiento).toLocaleDateString("es-ES", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="md:w-1/4 flex flex-col items-center">
              <div className="relative group">
                {pacienteData?.foto ? (
                  <Image
                    src={`http://127.0.0.1:8080/api/v1/view/${pacienteData.foto}`}
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg hover:bg-green-50 transition-colors duration-200">
                    ...
                    <div className="mt-3 space-y-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-700 font-medium">{pacienteData?.telefono || "No proporcionado"}</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg hover:bg-green-50 transition-colors duration-200">
                    ...
                    <div className="mt-3 space-y-3">
                      <div>
                        ...
                        <p className="text-gray-700 font-medium">{ultimaConsulta?.estatura ? `${ultimaConsulta.estatura} m` : "No hay datos"}</p>
                      </div>
                      <div>
                        <p className="text-xs font-medium text-gray-500">Peso</p>
                        <p className="text-gray-700 font-medium">{ultimaConsulta?.peso ? `${ultimaConsulta.peso} kg` : "No hay datos"}</p>
                      </div>
                      {ultimaConsulta?.estatura && ultimaConsulta?.peso && (
                        <div>
                          <p className="text-xs font-medium text-gray-500">IMC</p>
                          <p className="text-gray-700 font-medium">{(ultimaConsulta.peso / (ultimaConsulta.estatura * ultimaConsulta.estatura)).toFixed(1)}</p>
                        </div>
                      )}
                    </div>
                  </div>
